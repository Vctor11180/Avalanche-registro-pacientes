...
export const CONTRACT_ABIS = {
  AccessControl: [
    ...
    "function grantDocumentAccess(address _entity, uint8 _accessType, uint256[] _documentIds, uint256 _expiresAt, string _purpose) external",
    "function revokeAccess(address _entity) external",
    "function revokeDocumentAccess(address _entity, uint256[] _documentIds) external",
    "function checkPermission(address _patient, address _entity, uint8 _accessType) external view returns (bool)",
    "function checkDocumentPermission(address _patient, address _entity, uint256 _documentId, uint8 _accessType) external view returns (bool)",
    ...
    "function getDocumentPermissions(address _patient, address _entity) external view returns (uint256[])",
    "event AccessGranted(address indexed patient, address indexed entity, uint8 accessType, uint256 expiresAt, string purpose)",
    "event DocumentAccessGranted(address indexed patient, address indexed entity, uint256[] documentIds, uint8 accessType, uint256 expiresAt, string purpose)",
    "event AccessRevoked(address indexed patient, address indexed entity, uint256 revokedAt)",
    "event DocumentAccessRevoked(address indexed patient, address indexed entity, uint256[] documentIds, uint256 revokedAt)",
...
// Tipos de permisos (alcance)
export enum PermissionScope {
  ALL_DOCUMENTS = 0,
  SPECIFIC_DOCUMENTS = 1,
  DOCUMENT_CREATION = 2,
}
...
export const PERMISSION_SCOPE_LABELS = {
  [PermissionScope.ALL_DOCUMENTS]: 'Todos los Documentos',
  [PermissionScope.SPECIFIC_DOCUMENTS]: 'Documentos Específicos',
  [PermissionScope.DOCUMENT_CREATION]: 'Creación de Documentos',
};
